...
class AppConfigSpec extends UnitSpec {
  ...
  "endpointsEnabled" when {
    "the API version is disabled" should {
      "return false" in {
        ...
        val result = appConfigWithDisabledVersion.endpointsEnabled("3.0")
    ...
    "the API version is enabled" should {
      "return true" in {
        ...
        val result = appConfigWithEnabledVersion.endpointsEnabled("3.0")
  ...
  "endpointReleasedInProduction" when {
    "the API version is enabled and the config specifies the endpoint status as true" should {
      "return true" in {
        ...
        val result = appConfigWithReleasedVersion.endpointReleasedInProduction("3.0", "create-something")
        result shouldBe true
      }
    }
    ...
    "the API version is enabled and the config specifies the endpoint status as false" should {
      "return true" in {
        ...
        val result = appConfigWithDisabledEndpoint.endpointReleasedInProduction("3.0", "amend-something")
        result shouldBe false
      }
    }
    ...
    "the API version is enabled and the config doesn't specify the endpoint status" should {
      "return the value of endpointReleasedInProduction" in {
        ...
        val result = appConfigWithUnspecifiedEndpointStatus.endpointReleasedInProduction("3.0", "delete-something")
        result shouldBe true
      }
    }
  }
  ...
  "deprecationFor" when {
    "the API version is not deprecated" should {
      "return NotDeprecated" in {
        val config: AppConfigImpl = appConfig(
          """
            |    3.0 {
            |      status = "BETA"
            |      endpoints {
        ...
        config.deprecationFor(Version3) shouldBe Valid(NotDeprecated)
      }
    }
    ...
    "the API version is deprecated" when {
      "deprecatedOn is missing" should {
        "return the expected message" in {
          ...
          val result: Validated[String, Deprecation] = config.deprecationFor(Version3)

          result shouldBe Invalid("deprecatedOn date is required for a deprecated version 3.0")
        }
      }
  ...
  private def appConfig(versionConf: String): AppConfigImpl = {
    val conf = ConfigFactory.parseString(
      """
        ...
        """
          ...
          |        environmentHeaders = ["HIP-Accept", "HIP-Gov-Test-Scenario"]
          |      }
          |    }
          |  }
          |""".stripMargin
